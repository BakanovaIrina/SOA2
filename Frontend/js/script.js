...
function displayProductInfo(dataText) {
  if(numberStructure){
  ...
  if(tableStructure){
    ...
    `;
      ...
      const products = JSON.parse(dataText);
...
async function getByManufacturer(){
  ...
  await sendRequest("GET", `/manufacturer/${id}`, null, params, API_EBAY);
}
...
async function getListWithCurrentUnit(){
  ...
  await sendRequest("GET", `/unit-of-measure/${unit}`, null, params, API_EBAY);
}
